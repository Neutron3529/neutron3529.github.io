...
template<class T, int ALIGN> void Array<T, ALIGN>::create(int i) {
  n=reserved=ALIGN?(i+ALIGN-1)&(-ALIGN):i;
  if (i<=0) {
    data=0;
    ptr=0;
    return;
...
int dot_product(short *t, short *w, int n) {
  __m256i sum0={0,0,0,0},sum1={0,0,0,0};
  int res[8];
//  n=(n+31)&-32;
  for (int i=0; i<n; i+=32){
    sum0=_mm256_add_epi32(sum0,_mm256_srai_epi32(_mm256_madd_epi16(*(__m256i*)(t+i),*(__m256i*)(w+i)),8));
    sum1=_mm256_add_epi32(sum1,_mm256_srai_epi32(_mm256_madd_epi16(*(__m256i*)(t+i+16),*(__m256i*)(w+i+16)),8));
  }
  sum1=_mm256_add_epi32(sum0,sum1);
  sum1=_mm256_hadd_epi32(sum1,sum1);
  _mm256_store_si256((__m256i*)res, sum1);
...
/*
 int dot_product(short *t, short *w, int n) {
  ...
  n=(n+31)&-32;
  for (int i=0; i<n; i+=8){
    sum[0]+=(t[i]*w[i]+t[i+1]*w[i+1]) >> 8;
    sum[1]+=(t[i+2]*w[i+2]+t[i+3]*w[i+3]) >> 8;
    sum[2]+=(t[i+4]*w[i+4]+t[i+5]*w[i+5]) >> 8;
    sum[3]+=(t[i+6]*w[i+6]+t[i+7]*w[i+7]) >> 8;
  }
...
/*void train(short *t, short *w, int n, int err) {
  __m256i mm21,mm22,mm31,mm32,mm0;
  __m256i mm1={0x0001000100010001ULL,0x0001000100010001ULL,0x0001000100010001ULL,0x0001000100010001ULL};
  __m128i xmm0={0,0};
  int n2=n;
  n=(n+31)&-32;
  *(short*)&xmm0=err;
  mm0=_mm256_broadcastw_epi16(xmm0);
  for (int i=0; i<n; i+=32) {
    //    movdqa xmm2, [rdx+rcx*2] ; w[i]
    mm21=_mm256_load_si256((__m256i*)(w+i));
    mm22=_mm256_load_si256((__m256i*)(w+i+16));
    //    movdqa xmm3, [rax+rcx*2] ; t[i]
    mm31=_mm256_load_si256((__m256i*)(t+i));
    mm32=_mm256_load_si256((__m256i*)(t+i+16));
    //    paddsw xmm3, xmm3      ; t[i]*2
    mm31=_mm256_adds_epi16(mm31,mm31);
    mm32=_mm256_adds_epi16(mm32,mm32);
    //    pmulhw xmm3, xmm0      ; t[i]*err*2 >> 16
    mm31=_mm256_mulhi_epi16(mm31,mm0);
    mm32=_mm256_mulhi_epi16(mm32,mm0);
    //    paddsw xmm3, xmm1      ; (t[i]*err*2 >> 16)+1
    mm31=_mm256_adds_epi16(mm31,mm1);
    mm32=_mm256_adds_epi16(mm32,mm1);
    //    psraw xmm3, 1          ; (t[i]*err*2 >> 16)+1 >> 1
    mm31=_mm256_srai_epi32(mm31,1);
    mm32=_mm256_srai_epi32(mm32,1);
    //    paddsw xmm2, xmm3      ; w[i] + xmm3
    mm21=_mm256_adds_epi16(mm21,mm31);
    mm22=_mm256_adds_epi16(mm22,mm32);
    //    movdqa [rdx+rcx*2], xmm2
    _mm256_store_si256((__m256i*)(w+i),mm21);
    _mm256_store_si256((__m256i*)(w+i+16),mm22);
  }
}*/
void train(short *t, short *w, int n, int err) {
  __m256i mm21,mm22,mm31,mm32,mm0=_mm256_set1_epi16((short)err);
  __m256i mm1={0x0001000100010001ULL,0x0001000100010001ULL,0x0001000100010001ULL,0x0001000100010001ULL};
//  n=(n+31)&-32;
  for (int i=0; i<n; i+=32) {
    //    movdqa xmm2, [rdx+rcx*2] ; w[i]
    ...
    mm32=_mm256_adds_epi16(mm32,mm1);
    //    psraw xmm3, 1          ; (t[i]*err*2 >> 16)+1 >> 1
    mm31=_mm256_srai_epi16(mm31,1);
    mm32=_mm256_srai_epi16(mm32,1);
    //    paddsw xmm2, xmm3      ; w[i] + xmm3
    mm21=_mm256_adds_epi16(mm21,mm31);
    mm22=_mm256_adds_epi16(mm22,mm32);
    //    movdqa [rdx+rcx*2], xmm2
...
class Mixer {
  ...
  Array<short, 128> tx; // N inputs from add()
  Array<short, 128> wx; // N*M weights
